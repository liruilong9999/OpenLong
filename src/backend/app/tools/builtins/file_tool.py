...
class FileTool:
    spec = ToolSpec(
        name="file",
        description="Read, write, or create directories inside the agent workspace.",
        parameters=[
    ...
    async def run(self, **kwargs: Any) -> ToolResult:
        ...
        if action == "mkdir":
            ...
            return ToolResult(success=True, content=f"created dir: {Path(relative_path).as_posix()}")
